...
ApproximateClockToUnixTimeConverter::measurePair() {
  ...
  auto fast_0 = libkineto::getApproximateTime();
  auto wall = std::chrono::system_clock::now();
  auto fast_1 = libkineto::getApproximateTime();
...
ApproximateClockToUnixTimeConverter::measurePairs() {
  ...
  for (auto i = 0; i < n_warmup; i++) {
    libkineto::getApproximateTime();
    static_cast<void>(libkineto::steady_clock_t::now());
  }
...
std::function<time_t(libkineto::approx_time_t)> ApproximateClockToUnixTimeConverter::
    makeConverter() {
  ...
  return [=](libkineto::approx_time_t t_approx) {
...
void register_time_converter() {
  ...
  std::call_once(once, []() {
    ...
    libkineto::get_time_converter() = converter.makeConverter();
  });
}