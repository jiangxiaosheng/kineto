...
ThreadPool::ThreadPool(size_t num_threads) : stop(false) {
  if (num_threads == 0) {
    std::cerr
        << "Error: Number of threads cannot be zero. No worker threads created."
        << std::endl;
    stop = true; // Set stop flag to true to prevent enqueueing tasks.
    return;
  }
...
// Implementation of the ThreadPool destructor.
